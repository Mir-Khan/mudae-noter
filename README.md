...
- Pick colors visually with a color wheel on the **Colors** tab, scoped to your keyed characters, with detailed/compact/grid views (showing kakera value) and a "No Color Only" filter to find who's still missing one. Paste `$mmscy=` output there to refresh key counts and colors by name without re-pasting your whole collection.
...
- An **Ourosphere** tab tracks **Character Perks** (paste `$mmz=`/`$opp <name>`, or enter levels by hand) and account-wide **Shop** levels (paste `$ouroshop`, or by hand) as a filterable grid, and doubles as a planner - pick target levels across any characters to see a per-character and grand-total sphere cost, level several characters' perks at once via multi-select, and get a warning if a character's current perk levels cost more spheres than what you've tracked as invested. A tracked Perk 1 level also feeds into the Wishlists tab's booster math, which flags any Booster % with no tracked Perk 1 data as "unconfirmed", or as a mismatch if a tracked level actually disagrees with it.
...
- **Ourosphere and Colors tab improvements** - the perk detail panel is now compact ("Perk N" with the full description as a tooltip) so leveling up Perk 10 no longer needs a long scroll; a multi-select lets you level one perk across several characters at once; and a warning now shows if a character's current perk levels would cost more spheres than what you've tracked as invested. The Colors tab's character picker also gained detailed/compact/grid views (like the Sort tab) and now shows each character's last reported kakera value.
- **Update Key Counts & Colors** - a new box on the Colors tab: paste `$mmscy=` output (a much lighter listing than the full collection re-import) to refresh key counts and colors by name, without re-pasting your whole collection.
