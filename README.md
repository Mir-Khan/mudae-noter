...
- Save your own and your friends' `$wishlist` output on the **Wishlists** tab, and compare any two to see exactly which characters overlap - handy for trades or spotting who else wants what you've got. Each saved wishlist is a draggable row list with its own Star/Lock/Kakera-boost flags and a "Booster" toggle per character (disabled for anyone not claimed, since Ouroperks only exist on claimed characters) - a character's actual booster contribution is solved from the whole pasted list (their own shown `+N%` is what they *receive* from neighbors, not their own value, so it's inferred by solving the chain across the list rather than read directly) with no manual toggling needed, with live neighbor-boost indicators for Ouroperk-boosted characters - a shown `+N%` that includes a booster's own self-reflected part (a confirmed Mudae mechanic - "a part of the spawn chance bonus applied by perk 1 is also applied to the character upgraded") is recognized as such rather than flagged as a mystery, and can even confirm booster status when no neighbor solves the exact %; an optional "Ouroperk self-boost %" field (from `$shop`/`$ouroperks`) turns those otherwise-unknown percentages into a labeled estimate. A boost that genuinely doesn't fit any of this (researched: every other Mudae spawn-bonus source is flat/account-wide, not per-character) is flagged "unexplained" rather than guessed at - neighbor adjacency (both for solving and the live indicator) correctly wraps around the list, since the first and last characters are real neighbors of each other. Editing a row's flags automatically marks it "changed" and includes it the next time you generate a command, no separate step needed. Since `$wish`/`$sw`/etc. can only add characters and never actually reposition one that's already wished, dragging rows to reorder them alone changes nothing in Discord - a "Reorder Commands" section works out the minimal set of real `$wishi` (`$wishinsert`) commands needed to match the new order (characters whose relative order didn't change need no command at all), appearing only once the order actually diverges from the last known-real order. A built-in command builder generates the exact `$wish`/`$sw`/`$swlk`-style command for any typed name(s) *and*/or any changed/included rows, using each one's own flags - and if a command would `$starwish` past your saved capacity, it warns with the estimated wishlist-slot cost of Mudae's ramping upgrade price, the exact wishlist capacity you'd be left with, and - by name - which currently-wished characters would actually get pushed out by the shrunken capacity, before letting you generate it. A "This is my own wishlist" toggle in the modal (hidden for a friend's wishlist you're only tracking to compare) reveals capacity tracking: paste `$bonus` output to prefill wishlist/starwish/wishseries slot totals, correctly netted against any same-line deduction and with Mudae's free base allowance (7 wishlist / 1 starwish slot, never shown in `$bonus` itself) added automatically - still editable in case your account has some other unlisted modifier. It saves the moment you click Save Capacity (independent of saving the wishlist itself) and is shared automatically with every wishlist you flag "mine," so it's never re-entered per entry. Since `$wish`/`$sw`/etc. only ever add characters and never remove or overwrite anything, removing a row that was actually part of a real, previously-confirmed wishlist surfaces a ready-made `$wr` (`$wishremove`) command instead of silently pretending the real Discord wishlist changed too, and starting a brand new wishlist shows a reminder to run `$wra` (`$wishremoveall`) first if you want your real wishlist to end up matching exactly.
